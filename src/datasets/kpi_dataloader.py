...
class SingleWindowUnsupervisedKPIDataLoader(Dataset):
    def __init__(self, datatype, train_datapath, test_datapath, window_size, window_gap):
        ...
        ###############
        # Notice that event label are old version used to measure if a sequence with length of window size
        # is anomaly or not. However, according to the Precision and Recall on Time Series NIPS2018, there should
        # to use a different measurment for this kind of labels.
        if 'csv' in self.datapath:
            #### only process a designated single csv file.
            ...
            if (cfg.mode == 'unsupervised' and self.datatype == 'test') or cfg.have_label is False:
                # for now only in test mode because public dataset contains label for training data to valid results.
                # TODO rewrite this if used on huawei data.
            ...
            self.total_train_datasets, self.total_train_point_labels = [], []
            self.total_valid_datasets, self.total_valid_point_labels = [], []
            self.total_test_datasets, self.total_test_point_labels = [], []

            if self.datatype == 'train' or self.datatype == 'valid':
                    self.process_data(train_datasets_list=self.total_train_datasets,
                                    train_point_labels_list=self.total_train_point_labels,
                                    valid_datasets_list=self.total_valid_datasets,
                                    valid_point_labels_list=self.total_valid_point_labels)
            else:
                self.process_data(test_datasets_list=self.total_test_datasets,
                                  test_point_labels_list=self.total_test_point_labels)

        else:
            #### process list of csv files in the directory.
            ...
            for index, data_file in enumerate(os.listdir(self.datapath)):
                ...
                if self.datatype == 'train' or self.datatype == 'valid':
                    self.process_data(
                    train_datasets_list=self.total_train_datasets, train_point_labels_list=self.total_train_point_labels,
                    valid_datasets_list=self.total_valid_datasets, valid_point_labels_list=self.total_valid_point_labels)
            ...
            self.total_train_point_labels = np.array(self.total_train_point_labels)
            self.total_valid_point_labels = np.array(self.total_valid_point_labels)
            self.total_test_point_labels = np.array(self.total_test_point_labels)

            print('train_size: {}, train_point_labels_size: {}'.format(len(self.total_train_datasets),
                                                                       len(self.total_train_point_labels)))
            print('valid_size: {}, valid_point_labels_size: {}'.format(len(self.total_valid_datasets),
                                                                       len(self.total_valid_point_labels)))
            print('test_size: {}, test_point_labels_size: {}'.format(len(self.total_test_datasets),
                                                                       len(self.total_test_point_labels)))
    ...
    def process_data(self, train_datasets_list=None, train_point_labels_list=None,
                     ...
                     test_datasets_list=None, test_point_labels_list=None):
        values = self.values
        labels = self.labels
        ...
        if self.datatype == 'train' or self.datatype == 'valid':
            ...
            self._window_data_process(total_step=total_train_step,
                                    ...
                                    datasets=train_datasets_list,
                                       point_labels=train_point_labels_list)
            self._window_data_process(total_step=total_valid_step,
                                   values=valid_values,
                                   labels=valid_labels,
                                   datasets=valid_datasets_list,
                                    point_labels=valid_point_labels_list)

        else:   ## else would be test dataset.
            ...
            self._window_data_process(total_step=total_test_step,
                                     ...
                                     datasets=test_datasets_list,
                                      point_labels=test_point_labels_list
                                     )
    ...
    def _window_data_process(self, total_step, values, datasets, point_labels, labels=None):
        """
        constrcut data set into window_size and window_gap
        Args:
            return: dataset (List) and event_labels(List):
        ...
        for _ in range(total_step):
            ...
            if labels is not None:
                step_labels = labels[start_point: end_point]
                point_labels.append(step_labels)
...
class EventSingleWindowUnsupervisedKPIDataLoader(Dataset):
    def __init__(self, datatype, train_datapath, test_datapath, window_size, window_gap):
        self.window_size = window_size
        self.window_gap = window_gap
        self.datatype = datatype
        ...
        if 'csv' in self.datapath:
            ...
            ## for new point labels.
            self.total_valid_point_labels = []
            self.total_train_point_labels = []
            self.total_test_point_labels = []
